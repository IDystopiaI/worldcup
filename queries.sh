...
echo "$($PSQL "SELECT wteam.name from games JOIN teams AS wteam ON games.winner_id = wteam.team_id WHERE year=2014 AND round='Eighth-Final' UNION SELECT oteam.name FROM games join teams as oteam on games.opponent_id = oteam.team_id WHERE year=2014 AND round='Eighth-Final' ORDER BY name")"
